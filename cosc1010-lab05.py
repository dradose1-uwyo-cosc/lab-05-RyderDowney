...
for i in range(len(max_temps)):
    if max_temps[i] > largest:
        largest = max_temps[i]
print(f"The highest temperature recorded is {largest}")

smallest = min_temps[0]
for i in range(len(min_temps)):
    if min_temps[i] < smallest:
        smallest = min_temps[i]
print(f"The lowest temperature recorded is {smallest}")
